...
//    public $incrementing =false;
	...
	protected $fillable = ['goods_id', 'jooge_goods_id', 'bn', 'name', 'type_id', 'cat_id', 'brand_id', 'marketable', 'store', 'fav',
		...
		'pmt_id', 'goods_profit_ratio', 'is_pkg', 'pkg_info', 'service_into', 'mktprice'
	];
...
//				foreach ($goods['mechanics'] as $mechanic)
	...
	{
		$goodsData = Good::with('cartObjects', 'image_attach', 'images', 'mechanics', 'goods_ports',
			'assemblies', 'standardfits', 'electrics', 'aspect_pics', 'mechanics_inte', 'electrics_inte'
		)->whereIn('goods_id', $ids)->get();//->toArray();
		foreach ($goodsData as $dataK => $data) {
			foreach ($data['image_attach'] as $itemK => $item) {
				...
				$goodsData[$dataK]['image_attach'][$itemK] = $collapse->toArray();
			}
		}
		return $goodsData;
	}
