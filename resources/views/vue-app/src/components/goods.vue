    ...
    <div style="height:100%">
      ...
      <div class="goods-desc" style="height:100%;padding-bottom:3rem;box-sizing:border-box">
        ...
        <div v-if="index==1" class="params-cell">
          ...
          <div @click="collapse(3)" v-if="goods_data_list.new_standardfits.has_item" :class="{'border-1px-b':!collapse3}" class="collapse_title color-danger bg-sliver padding-rl-10 padding-tb-6">
            标准配件
          ...
          <group class="margin-0" v-for="(item,index) in goods_data_list.new_standardfits" v-show="collapse3">
            <cell class="font-mini" :title="parms_table.standardfits[index]||index" v-if="index!='created_at'&&index!='updated_at'&&index!='goods_id'&&index!='id'&&index!='type'&&index!='has_item'">
              <flexbox :gutter="0" slot="value" class="text-center">
                <flexbox-item v-for="(_item,_index) in item">
                  {{_item}}
                </flexbox-item>
          ...
          <div @click="collapse(4)" v-if="goods_data_list.new_assemblies.has_item" :class="{'border-1px-b':!collapse4}" class="collapse_title color-danger bg-sliver padding-rl-10 padding-tb-6">
            可选组件
          ...
          <group class="margin-0" v-for="(item,index) in goods_data_list.new_assemblies" v-show="collapse4">
            <cell class="font-mini" :title="parms_table.assemblies[index]||index" v-if="index!='created_at'&&index!='updated_at'&&index!='goods_id'&&index!='id'&&index!='type'&&index!='has_item'">
              <flexbox :gutter="0" slot="value" class="text-center">
                <flexbox-item v-for="(_item,_index) in item">
                  {{_item}}
                </flexbox-item>
  ...
  export default {
    ...
    methods: {
      ...
      init_goods_page: function(query) {
        ...
        api.getGoodsData({
          relations: ["image_attach", "images", "mechanics", "goods_ports", "assemblies", "standardfits", "electrics", "aspect_pics", "mechanics_inte", "electrics_inte"],
          parameters: query
        }).then((res) => {
